	...
	$.each(["Customer", "Supplier", "Company"], function (i, doctype) {
		frappe.ui.form.on(doctype, {
			refresh: function (frm) {
				frm.set_query("license", "licenses", function (doc, cdt, cdn) {
					var set_licenses = doc.licenses.map(function (license) { return license.license; });
					return {
	...
	*/
	...
	erpnext.stock.StockController = frappe.ui.form.Controller.extend({
		...
		package_tag: function(doc, cdt, cdn) {
			if (["Sales Order", "Sales Invoice", "Delivery Note", "Stock Entry"].includes(doc.doctype)) {
				var row = this.frm.selected_doc || locals[cdt][cdn];

				// set batch no from the selected package tag, even if none found
				if (row.package_tag) {
					frappe.db.get_value("Package Tag", row.package_tag, "batch_no", function (r) {
	...
	erpnext.taxes_and_totals = erpnext.payments.extend({
		...
		set_and_update_excise_tax: function () {
			...
			var me = this;
			frappe.db.get_value("Compliance Info", { "name": this.frm.doc.license }, "license_for", function (r) {
